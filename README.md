...
| 正常終了 | `last_turn_outcome.status == "completed"` かつ最終回答が watchdog 固有の `doneToken` と完全一致 → ナッジせず監視終了 |
...
ナッジ本文は `[KEEP-AWAKE n/max]` タグ付きの短文で、「中断箇所から続行」と、完了時は `doneToken` だけを返すことを明記します。`doneToken` は省略時に watchdog ごとの一意な値を自動生成し、組み込み文・`message` によるカスタム文のどちらにも必ず追記します。成功扱いで監視を終了する条件は、完成した最終回答の全体（前後空白を除く）がこのトークンと完全一致することです。通常の完了回答だけでは終了せず、idle 後に再度ナッジします。期限切れ・予算枯渇・手動停止は安全弁として従来どおり終了します。
