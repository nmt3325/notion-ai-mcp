...
| 正常終了 | `last_turn_outcome.status == "completed"` かつ最終回答の最後の空でない行が watchdog 固有の `doneToken` と完全一致 → ナッジせず監視終了 |
...
ナッジ本文は `[KEEP-AWAKE n/max]` タグ付きの短文で、「中断箇所から続行」と、完了時は最終回答の最後の空でない行に `doneToken` を単独で置くことを明記します。トークンより前には成果報告などの文章を書けます。`doneToken` は省略時に watchdog ごとの一意な値を自動生成し、組み込み文・`message` によるカスタム文のどちらにも必ず追記します。成功扱いで監視を終了する条件は、完成した最終回答の最後の空でない行（前後空白を除く）がこのトークンと完全一致することです。通常の完了回答だけでは終了せず、idle 後に再度ナッジします。期限切れ・予算枯渇・手動停止は安全弁として従来どおり終了します。
...
ステップ上限で止まったターンは lease を解放しないまま `last_turn_outcome` も書かれないため、`completed` を待つ判定だけでは検知できません。heartbeat が `idleMs` 以上凍結していて lease が残っている場合は、その時点で反復数の証拠を読み、上限に達していればテキストナッジでなく Continue と同じ継続リクエストを送ります。この経路では lease の中断を行いません（中断すると Continue が再開すべきチェックポイントを捨てるため）。
...
ステップ上限かどうかは、ネイティブ UI と同じ数え方で判定します。UI が数えているのは `step_count` ではなく、そのターンの `agent-inference` ステップ数で、スクリプトエージェント有効なら 100、トリガー起動なら 50、それ以外は 15 が上限です。ターンの境界は直近のユーザーステップなので、継続した実行は前の実行が使った反復数を引き継ぎます（実測: 1 ターンに 127 反復 = 最初の実行 101 + 継続後 26）。反復数が読めない履歴では、`step_count` が `NOTION_KEEP_AWAKE_STEP_LIMIT_STEPS`（既定 2000）以上ならステップ上限と見るフォールバックに落ちます。上限と判定できなければ早死にと見て通常のナッジを送ります。どちらの場合も `completed` を完走扱いにせず監視を続けます。文言一致は、将来 Notion がプロンプトを本文に書き込むようになった場合の保険として残しています。

最終ステップは 1 レコードの追加取得です。確認は現在のターンが閉じた場合や沈黙時に限ります。文言照合用の履歴取得は補助で、保存された最終ステップを優先します。反復数の読み取りは上限に達した時点で打ち切るため、数千ステップのターンでも追加リクエストは数件で済みます。2000 ステップの閾値は反復数が読めない場合のフォールバックとして残した推定値で、公式の停止理由フィールドではありません。
