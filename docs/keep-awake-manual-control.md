...
Only `conversationId` is required; omitting `doneToken` generates a unique one and returns it in `keepAlive.doneToken`. Successful completion requires the assistant reply's final non-empty line to equal that token exactly; prose may precede it, and every built-in or custom nudge includes it. Bounds: `idleSeconds` 60–900, `pollSeconds` 5–300,
