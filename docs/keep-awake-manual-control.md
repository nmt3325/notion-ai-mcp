...
Only `conversationId` is required; omitting `doneToken` generates a unique one and returns it in `keepAlive.doneToken`. Successful completion requires the assistant reply's final non-empty line to end with that token; prose may precede it on the same or earlier lines, and every built-in or custom nudge includes it. Bounds: `idleSeconds` 60–900, `pollSeconds` 5–300,
