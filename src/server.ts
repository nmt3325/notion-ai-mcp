...
export function createServer(client: NotionClient, shared?: { keepAwake?: KeepAwakeSupervisor | undefined }): McpServer {
  ...
  server.registerTool("keep_me_awake", {
    ...
    description: [
      ...
      "A successful completion requires the assistant's entire final reply to exactly equal this watch's doneToken. A normal finished answer without that token remains watched and is nudged after the idle window.",
    ...
    inputSchema: {
      ...
      doneToken: z.string().trim().min(3).max(64).optional().describe("Exact assistant reply required to complete the watch. A unique token is generated when omitted, and every nudge quotes it."),
    ...
    return result(record, `Watching ${record.conversationId}. Completion requires the exact reply ${record.doneToken}; every nudge includes it. Nudges after ${seconds(record.idleMs)}s of silence, at most ${record.maxNudges} times, until ${new Date(record.deadlineAt).toISOString()}. ${continues} Stop it with stop_keep_me_awake and keepAliveId ${record.keepAliveId}.`);
  });
